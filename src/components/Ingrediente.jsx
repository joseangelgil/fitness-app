...
const Ingrediente = () => {
  ...
  return (
    <Stack direction='row' alignItems='flex-start' justifyContent='center' flexWrap='wrap' gap='50px' padding='50px 10px'>
      {data.map((item,index) => {
        return (
          <Stack key={index} className='ingredient-card' justifyContent= 'space-evenly' alignItems='center' width='250px' height='350px' sx={{borderRadius:'10px', boxShadow:'0 2px 3px 1px rgba(0,0,0,0.7)', cursor: 'pointer'}} onClick={() => handleIngredientClick(item.id)}>
